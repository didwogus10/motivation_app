class FamousListModel {
  ...
  void remove_number() {
    // famous_saying.shuffle();

    for (int i = 0; i < famous_saying.length; i++) {
      famous_saying[i] =
          famous_saying[i].replaceAll(RegExp(r'[0-9]'), ''); // 명언마다 앞에 숫자 제거
      famous_saying[i] =
          famous_saying[i].replaceFirst('.', '').trimLeft(); // 명언마다 앞에 . 제거
      famous_saying[i] = famous_saying[i].replaceAll('–', '\n\n–');
      famous_saying[i] = famous_saying[i] + ' –';
    }
  }
}
// 몇 글자 이상 넘으면 ... 으로 변경